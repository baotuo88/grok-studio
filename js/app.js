...
function initElements() {
  AppState.elements = {
    ...
    registerConfirmPasswordGroup: document.getElementById('registerConfirmPasswordGroup'),
...
async function sendChatMessage() {
  ...
  // 检查配额
  try {
    await UserManager.consumeUsage('chat');
  } catch (e) {
...
async function generate() {
  ...
  // 检查配额
  try {
    await UserManager.consumeUsage('image');
  } catch (e) {
    UI.showError(e.message);
    return;
  }
...
function showLoginModal() {
  ...
  AppState.elements.registerConfirmPassword.value = '';
  AppState.elements.rememberMe.checked = UserManager.isRememberMeEnabled();
  setAuthMode('login');
...
function setAuthMode(mode = 'login') {
  ...
  const isAdminUsername = username === UserManager.ADMIN_USERNAME;

  if (isAdminUsername && mode === 'register') {
    mode = 'login';
  }
  ...
  AppState.elements.switchToLogin.classList.toggle('btn-primary', !isRegisterMode);
  AppState.elements.switchToLogin.classList.toggle('btn-secondary', isRegisterMode);
  AppState.elements.switchToRegister.classList.toggle('btn-primary', isRegisterMode);
  AppState.elements.switchToRegister.classList.toggle('btn-secondary', !isRegisterMode);
  AppState.elements.switchToRegister.disabled = isAdminUsername;

  AppState.elements.registerConfirmPasswordGroup.classList.toggle('hidden', !isRegisterMode);
  AppState.elements.loginBtn.textContent = isRegisterMode ? '注册并登录' : '登录';

  if (isAdminUsername) {
    AppState.elements.authModeHint.textContent = 'admin 为系统内置管理员账号，仅支持登录';
  } else if (isRegisterMode) {
    AppState.elements.authModeHint.textContent = '注册成功后会自动登录并进入系统';
  } else {
    AppState.elements.authModeHint.textContent = '已有账号直接登录，新用户请切换到注册模式';
  }
}
...
async function handleLogin() {
  ...
  const confirmPassword = AppState.elements.registerConfirmPassword.value;
  const rememberMe = AppState.elements.rememberMe.checked;
  const isRegisterMode = AppState.authMode === 'register';
  ...
  try {
    if (isRegisterMode) {
      await UserManager.register(username, password, confirmPassword, rememberMe);
    } else {
      await UserManager.login(username, password, rememberMe);
    }
...
async function showUserManagement() {
  ...
  const quotas = await Promise.all(
    users.map(async (username) => [username, await UserManager.getUserQuotaAsync(username)])
  );
  ...
  users.forEach(username => {
    const quota = quotaMap.get(username) || UserManager.getDefaultQuota();
 ...
 * 更新用户配额
 */
window.updateUserQuota = async function(username) {
  ...
  try {
    const quota = await UserManager.getUserQuotaAsync(username);
    quota.chatLimit = chatLimit;
    quota.imageLimit = imageLimit;
    await UserManager.setUserQuota(username, quota);
    UI.showError('配额已更新');
...
async function initApp() {
  ...
  if (!UserManager.isLoggedIn()) {
    ...
    AppState.elements.switchToLogin.addEventListener('click', () => setAuthMode('login'));
    AppState.elements.switchToRegister.addEventListener('click', () => setAuthMode('register'));
    ...
    AppState.elements.loginUsername.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        if (!AppState.elements.loginPassword.value.trim()) {
          e.preventDefault();
    ...
    AppState.elements.loginPassword.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        if (AppState.authMode === 'register') {
          e.preventDefault();
          AppState.elements.registerConfirmPassword.focus();
        } else {
          handleLogin();
        }
      }
    });
    AppState.elements.registerConfirmPassword.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        handleLogin();
      }
    });
